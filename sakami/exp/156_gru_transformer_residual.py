...
class SleepModel(nn.Module):
    def __init__(self, model_name: str, numerical_input_size: int, categorical_input_size: int) -> None:
        super().__init__()
    ...
    def forward(self, num_x: torch.FloatTensor, cat_x: torch.LongTensor) -> torch.FloatTensor:
        ...
        res = self.transformer(x, attention_mask=torch.ones_like(x)[:, :, 0]).last_hidden_state
        x = x + res
...
def predict(model: nn.Module, data_loader: DataLoader, preds_truncate_size: int, use_amp: bool = True) -> pl.DataFrame:
    model.eval()
    device = model.device
