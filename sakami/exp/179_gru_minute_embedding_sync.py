...
class SleepModel(nn.Module):
    ...
    def forward(self, num_x: torch.FloatTensor, cat_x: torch.LongTensor) -> torch.FloatTensor:
        ...
        minute_embedding = self.category_embeddings[0](cat_x[:, :, 0])
...
def main(debug: bool = False) -> None:
    ...
    with timer("preprocess"):
        ...
        tolerance_steps = list(range(12, 132, 12))
        ...
        categorical_columns = ["minute", "hour", "weekday", "periodicity"]
        numerical_columns = train_df.select(r"^(anglez|enmo).*$").columns

        assert categorical_columns[0] == "minute"
