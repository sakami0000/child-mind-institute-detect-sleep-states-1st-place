    ...
    with torch.inference_mode():
        for series_row_ids, num_x, cat_x, attention_mask, _ in tqdm(data_loader, desc="predict", leave=False):
            ...
            pred = (
                ...
                .sigmoid()
                .cpu()
            )  # (batch_size, sequence_length - preds_truncate_size * 2, 3)
            pred_mask = attention_mask.unsqueeze(-1).expand(-1, -1, 3).bool()
            pred = pred.masked_select(pred_mask).view(-1, 3).numpy()
            preds.append(pred)
    ...
    preds_df = pl.DataFrame(
        {
            ...
            "sleep": np.concatenate(preds, axis=0)[:, 2],
        }
    )
...
def main(debug: bool = False) -> None:
    ...
    with timer("preprocess"):
        ...
        train_df = (
            ...
            .with_columns(
                (
                    pl.col("full_event_onset").cumsum().over("series_id")
                    - pl.col("full_event_wakeup").cumsum().over("series_id")
                ).alias("sleep_label"),
            )
            .drop(["full_event_onset", "full_event_wakeup"])
        )
        ...
        # loss weight
        loss_weight = torch.FloatTensor([[[1.0, 1.0, config.sleep_label_weight]]]).to(config.device)
