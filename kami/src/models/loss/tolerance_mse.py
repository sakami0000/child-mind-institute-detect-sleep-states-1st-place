...
class ToleranceMSELoss(nn.Module):
    def __init__(
        self,
        ...
        super(ToleranceMSELoss, self).__init__()
    ...
    def forward(self, logits, labels, masks):
        ...
        loss = torch.square(
            torch.relu(logits_event.unsqueeze(1) - masked_logits_max.unsqueeze(2))
        )  # (batch, tolerance, seq, class)
