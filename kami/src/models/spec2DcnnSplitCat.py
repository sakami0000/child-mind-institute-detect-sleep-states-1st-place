...
class Spec2DCNNSplitCat(nn.Module):
    def __init__(
        self,
        cfg: DictConfig,
        ...
        self.cat_features = sorted(list(cfg.cat_features.keys()))
        self.embeddings = nn.ModuleDict(
            {
                name: nn.Embedding(feat_dict["num_category"], feat_dict["dim"])
                for name, feat_dict in cfg.cat_features.items()
            }
        )
    ...
    def forward(
        ...
        embedded = [self.embeddings[feat](x_cat[:, i, :]) for i, feat in enumerate(self.cat_features)]
        embedded = torch.cat(embedded, dim=2).permute(0, 2, 1)  # (batch_size, n_cat_features, n_timesteps)
