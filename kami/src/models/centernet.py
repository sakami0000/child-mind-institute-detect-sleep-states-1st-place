...
class CenterNet(BaseModel):
    ...
    def _logits_to_proba_per_step(self, logits: torch.Tensor, org_duration: int) -> torch.Tensor:
        ...
        proba_per_step = np.zeros((bs, org_duration, 3))
        for i in range(bs):
            ...
            f_onset = interp1d(pred_onset_pos_i, pred_onset_i, kind="linear", fill_value=0, bounds_error=False)
            f_wakeup = interp1d(pred_wakeup_pos_i, pred_wakeup_i, kind="linear", fill_value=0, bounds_error=False)

            proba_per_step_onset_i = f_onset(x)
            proba_per_step_wakeup_i = f_wakeup(x)
            proba_per_step[i, :, 1] = proba_per_step_onset_i
            proba_per_step[i, :, 2] = proba_per_step_wakeup_i
