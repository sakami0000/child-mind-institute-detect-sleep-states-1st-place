...
def get_train_dataloader(
    cfg: DictConfig, fold: int | None, stride: int = 0
) -> DataLoader:
...
class SegModel(LightningModule):
    def __init__(
        ...
        valid_duration = nearest_valid_size(
    ...
    def __share_step(self, batch, mode: str) -> torch.Tensor:
        if mode == "train":
            ...
            output = self.model(
                batch["feature"], batch["label"], batch["masks"], do_mixup, do_cutmix
            )
        elif mode == "val":
            do_mixup = False
            do_cutmix = False
        ...
        if mode == "train":
            ...
            if self.cfg.model.name == "CenterNet":
                resized_logits = (
                    self.model._logits_to_proba_per_step(logits, self.duration)
                    .detach()
                    .cpu()
                )
            else:
    ...
    def save_train_pred(self):
        ...
        for series_id in unique_series_ids:
            ...
            # 閾値以下を0にする
            this_series_preds[
                this_series_preds < self.cfg.label_correct.pred_threshold
            ] = 0.0
            series2preds[series_id] = (
                this_series_preds * self.cfg.label_correct.pred_rate
            )
    ...
    def on_validation_epoch_end(self):
        ...
        if ((self.cfg.monitor == "val_score") and (score > self.__best_score)) or (
            ...
            model_dict = (
                ...
                else self.model_ema.module.state_dict()
            )
    ...
    def configure_optimizers(self):
        ...
        num_warmup_steps = (
            math.ceil(self.trainer.max_steps / self.cfg.epoch) * 1
            if self.cfg.scheduler.use_warmup
            else 0
        )
