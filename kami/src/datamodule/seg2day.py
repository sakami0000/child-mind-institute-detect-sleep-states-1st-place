...
class TrainDataset(Dataset):
    def __init__(
        ...
        self.cfg = cfg
        self.day_duration = 24 * 60 * 12  # 24h * 60min * 12step/min
        ...
        self.upsampled_num_frames = (
            nearest_valid_size(int(self.day_duration * self.cfg.upsample_rate), self.cfg.downsample_rate) * 2
        )
    ...
    def __getitem__(self, idx):
        ...
        # crop
        start, end = random_crop(pos, self.day_duration * 2, n_steps)  # pos を含む duration の範囲でランダムにcrop
        ...
        label, masks = get_label(this_event_df, num_frames, self.day_duration * 2, start, end)
...
class ValidDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        start = chunk_id * self.day_duration * 2
        end = start + self.day_duration * 2
        num_frames = self.upsampled_num_frames // self.cfg.downsample_rate
...
class TestDataset(Dataset):
    def __init__(
        ...
        self.num_features = len(cfg.features)
        self.upsampled_num_frames = (
            nearest_valid_size(int(self.cfg.duration * self.cfg.upsample_rate), self.cfg.downsample_rate) * 2
        )
