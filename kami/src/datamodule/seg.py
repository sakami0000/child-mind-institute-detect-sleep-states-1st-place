...
def load_features(
    ...
    for series_id in series_ids:
        ...
        for feature_name in feature_names:
            ...
            if (periodicity_dict is not None) and (
                ("_cos" not in feature_name) and ("_sin" not in feature_name)
            ):
...
def load_chunk_features(
    ...
    for series_id in series_ids:
        ...
        this_feature = pad_if_needed(
            this_feature, stride + num_chunks * duration, pad_value=0
        )
        for i in range(num_chunks):
            chunk_feature = this_feature[
                stride + i * duration : stride + (i + 1) * duration
            ]
    ...
    for onset, wakeup in this_event_df[["onset", "wakeup"]].to_numpy():
        ...
        if onset >= 0 and onset < num_frames:
            ...
            for i, tolerance in enumerate(tolerances):
                masks[
                    i, max(0, onset - tolerance) : min(num_frames, onset + tolerance), 0
                ] = 1
...
class TrainDataset(Dataset):
    ...
    def __len__(self):
        return 10 if self.cfg.debug else len(self.event_df)
    ...
    def __getitem__(self, idx):
        ...
        # sample background
        if (random.random() < self.cfg.bg_sampling_rate) and (
            series_id not in self.cfg.ignore.negative
        ):
            pos = negative_sampling(this_event_df, n_steps)

        # crop
        start, end = random_crop(
            pos, self.cfg.duration, n_steps
        )  # pos を含む duration の範囲でランダムにcrop
...
class SegDataModule(LightningDataModule):
    def __init__(self, cfg: DictConfig, fold: int | None = None):
        ...
        ).filter(~pl.col("series_id").is_in(self.cfg.ignore.train))
