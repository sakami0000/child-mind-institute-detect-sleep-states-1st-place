...
def load_data() -> pl.DataFrame:
    ...
    series_df = series_df.with_columns(pl.col("timestamp").dt.truncate("1m"))
...
def main() -> None:
    ...
    with timer("load_data"):
        ...
        pred_df = pl.concat([sakami_df, kami_df, series_df], how="horizontal")
        pred_df = pred_df.group_by(["series_id", "timestamp"], maintain_order=True).agg(
        ...
        pred_df = pred_df.with_columns(
            ...
            (
                pl.min_horizontal([f"pred_onset_{model_name}" for model_name in model_names])
                / len(model_names)
            ).alias("prediction_onset_min"),
            (
                pl.max_horizontal([f"pred_onset_{model_name}" for model_name in model_names])
                / len(model_names)
            ).alias("prediction_onset_max"),
            (
                pl.sum_horizontal([f"pred_wakeup_{model_name}" for model_name in model_names])
                / len(model_names)
            ).alias("prediction_wakeup"),
            (
                pl.min_horizontal([f"pred_wakeup_{model_name}" for model_name in model_names])
                / len(model_names)
            ).alias("prediction_wakeup_min"),
            (
                pl.max_horizontal([f"pred_wakeup_{model_name}" for model_name in model_names])
                / len(model_names)
            ).alias("prediction_wakeup_max"),
        )
