...
import pandas as pd
import polars as pl
from components.factories.tree_factory import LGBMModel
...
def main(config: DictConfig):
    ...
    event_df = pl.read_csv("./input/train_events.csv").filter(pl.col("step").is_not_null())
    for event in ["onset", "wakeup"]:
        df = df.with_columns((pl.col("event") == event).cast(int).fill_null(0).alias("label"))
        print(df["label"].value_counts())
        model = LGBMModel(config.lgbm, event_df, event_name=event)
        df = model.cv(df)
    ...
    mean_ap = event_detection_ap(event_df.to_pandas(), sub_df.to_pandas())
