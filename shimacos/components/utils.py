...
def load_periodicity() -> pl.LazyFrame:
    periodicity_df = []
    for path in glob("../kami/processed/train/*/periodicity.npy"):
        periodicity = np.load(path)
        series_id = path.split("/")[-2]
        periodicity_df.append(
            {
                "series_id": series_id,
                "step": range(len(periodicity)),
                "periodicity": periodicity.tolist(),
            }
        )
    periodicity_df = pl.DataFrame(periodicity_df)
    periodicity_df = periodicity_df.explode(["step", "periodicity"])
    return periodicity_df.lazy()


def make_submission(
    row_idxs: dict[str, Any],
    preds: np.ndarray,
    steps: np.ndarray,
...
def post_process_from_2nd(
    ...
    height: float = 0.001,
    event2col: dict[str, str] = {
        "onset": "stacking_prediction_onset",
        "wakeup": "stacking_prediction_wakeup",
    },
    ...
    for event, event_pred_col in event2col.items():
        ...
        if use_daily_norm:
            minute_pred_df = (
                minute_pred_df.with_columns(pl.col("timestamp").dt.offset_by(event2offset[event]).dt.date().alias("date"))
                .with_columns(pl.col(event_pred_col).sum().over(["series_id", "date"]).alias("date_sum"))
                .with_columns(pl.col(event_pred_col) / (pl.col("date_sum") + (1 / (daily_score_offset + pl.col("date_sum")))))
            )
        ...
        minute_pred_df = minute_pred_df.with_columns(
            pl.sum_horizontal(
                [
                    (
                        pl.col(event_pred_col).rolling_sum(window_size=window, center=False, min_periods=1).over(["series_id", "chunk_id"])
                        / match_sums[i]
                    )
        ...
        for series_id, series_df in tqdm(
            ...
            steps = series_df["step"].to_numpy(writable=True)
...
def remove_periodicity(
    ...
    for series_id, series_df in df.group_by("series_id"):
        for _event, event_pred_col in event2col.items():
...
def post_process(
    ...
    for event, event_pred_col in event2col.items():
        ...
        minute_pred_df = minute_pred_df.with_columns(
            pl.sum_horizontal(
                [
                    (pl.col(event_pred_col).rolling_sum(window_size=window, center=False, min_periods=1).over("series_id"))
                    for window in high_match_nums
                ]
            ).alias(f"{event}_left_expectation_plus_3step"),
            pl.sum_horizontal(
                [(pl.col(event_pred_col).rolling_sum(window_size=window, center=False, min_periods=1).over("series_id")) for window in low_match_nums]
            ).alias(f"{event}_left_expectation_plus_9step"),
        )
        ...
        minute_pred_df = minute_pred_df.with_columns(
            pl.sum_horizontal(
                [
                    (pl.col(event_pred_col).shift(1).rolling_sum(window_size=window, center=False, min_periods=1).over("series_id").fill_null(0))
